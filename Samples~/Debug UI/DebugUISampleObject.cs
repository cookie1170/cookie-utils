...
namespace CookieUtils.Samples.DebugUI
{
    public class DebugUISampleObject
        : MonoBehaviour,
            IDebugDrawer,
            IPointerDownHandler,
            IPointerUpHandler
    {
        public void SetUpDebugUI(IDebugUI_BuilderProvider provider)
        {
            ...
            transformGroup.Vector3Field(
                "Position",
                () => transform.position,
                val => rb.MovePosition(val)
            );
            transformGroup.IntField(
                "Rotation",
            ...
            rigidbodyFoldout.Vector2Field(
                "Velocity",
                () => rb.linearVelocity,
                val => rb.linearVelocity = val
            );
            rigidbodyFoldout.FloatField(
                "Angular velocity",
                () => rb.angularVelocity,
                val => rb.angularVelocity = val
            );
        ...
        [SerializeField]
        private float deltaWeight = 10f;

        [SerializeField]
        private float posWeight = 10f;
