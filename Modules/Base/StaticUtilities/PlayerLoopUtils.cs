    ...
    public static class PlayerLoopUtils
    {
        public static void RemoveSystem(ref PlayerLoopSystem loop, in PlayerLoopSystem system)
        {
            ...
            for (int i = 0; i < systems.Count; i++)
            {
                ...
                if (
                    subSystem.type == system.type
                    && subSystem.updateDelegate == system.updateDelegate
                )
                {
                    systems.RemoveAt(i);
        ...
        private static void HandleSubsystemRemove(
            ref PlayerLoopSystem loop,
            in PlayerLoopSystem system
        )
        {
            for (int i = 0; i < loop.subSystemList.Length; i++)
                RemoveSystem(ref loop.subSystemList[i], in system);
        }
        ...
        )
        {
            ...
            if (loop.subSystemList != null)
                systemList.AddRange(loop.subSystemList);
        ...
        private static bool HandleSubsystemInsert<T>(
            ref PlayerLoopSystem loop,
            in PlayerLoopSystem system,
            int index
        )
        ...
        private static void HandleSubsystemPrint(
