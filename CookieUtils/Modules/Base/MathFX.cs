...
namespace CookieUtils 
{
	...
	public static class Mathfx 
    {
        ...
        public static Vector2 Hermite(Vector2 start, Vector2 end, float value) => 
			new(Hermite(start.x, end.x, value), Hermite(start.y, end.y, value));

        public static Vector3 Hermite(Vector3 start, Vector3 end, float value) =>
            new(Hermite(start.x, end.x, value), Hermite(start.y, end.y, value), Hermite(start.z, end.z, value));
        ...
        public static Vector2 Sinerp(Vector2 start, Vector2 end, float value) =>
            new(Mathf.Lerp(start.x, end.x, Mathf.Sin(value * Mathf.PI * 0.5f)), Mathf.Lerp(start.y, end.y, Mathf.Sin(value * Mathf.PI * 0.5f)));

        public static Vector3 Sinerp(Vector3 start, Vector3 end, float value) =>
            new(Mathf.Lerp(start.x, end.x, Mathf.Sin(value * Mathf.PI * 0.5f)), Mathf.Lerp(start.y, end.y, Mathf.Sin(value * Mathf.PI * 0.5f)), Mathf.Lerp(start.z, end.z, Mathf.Sin(value * Mathf.PI * 0.5f)));
        ...
        public static Vector2 SmoothStep(Vector2 vec, float min, float max) =>
            new(SmoothStep(vec.x, min, max), SmoothStep(vec.y, min, max));

        public static Vector3 SmoothStep(Vector3 vec, float min, float max) =>
            new(SmoothStep(vec.x, min, max), SmoothStep(vec.y, min, max), SmoothStep(vec.z, min, max));

        public static float Lerp(float start, float end, float value) =>
			((1.0f - value) * start) + (value * end);
        ...
        public static Vector2 Bounce(Vector2 vec) =>
            new(Bounce(vec.x), Bounce(vec.y));

        public static Vector3 Bounce(Vector3 vec) =>
            new(Bounce(vec.x), Bounce(vec.y), Bounce(vec.z));

        public static bool Approx(float val, float about, float range) =>
			((Mathf.Abs(val - about) < range));
